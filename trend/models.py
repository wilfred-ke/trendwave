...
ext_validator = FileExtensionValidator(
    ['.png', '.jpg', '.jpeg', '.mp4', '.mkv', '.webm', '.avi', '.mov', '.gif'])
User = get_user_model()


def validate_file_mimetype(file):
    accept = ['image/png', 'image/jpg', 'image/jpeg', 'video/mp4', 'video/webm',
              'video/avi', 'video/mov', 'video/gif', 'image/gif', 'video/mkv']
...
class TheProfile(models.Model):
    ...
    profileimg = models.ImageField(
        upload_to="profile_images/", default='blank_pic.jpg')
    ...
    def get_likes_count(self):
        user_images = Image.objects.filter(user=self.user)
        likes_count = LikeImage.objects.filter(
            image_id__in=user_images.values('id')).count()
        return likes_count
...
class Image(models.Model):
    ...
    file = models.FileField(upload_to="post_files/", null=False,
